...
import { Search, Receipt, TrendingUp, Loader2, Eye, RefreshCw, IndianRupee, CreditCard, Banknote, Filter } from 'lucide-react';
...
export default function PosSales() {
  ...
  const fetchSales = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          point_of_sale_customers(name, phone)
        `)
        .eq('user_id', user.id)
  ...
  const filteredSales = useMemo(() => {
    ...
    if (billingFilter !== 'all') {
      result = result.filter(sale => sale.billing_type === billingFilter);
    }
    ...
    if (debouncedSearchTerm) {
      ...
      result = result.filter(sale => {
        ...
        const docNum = (sale.billing_type === 'gst_invoice' ? sale.invoice_number : sale.bill_number)?.toLowerCase() || '';
        ...
        return customerName.includes(lowerSearch) ||
               ...
               saleId.includes(lowerSearch) ||
               docNum.includes(lowerSearch) ||
               customerGstin.includes(lowerSearch) ||
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent className="p-5">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold">{formatPrice(metrics.totalRevenue)}</h3>
              </div>
          ...
          <CardContent className="p-5">
            <div className="flex justify-between items-start">
             ...
             <div className="mt-4 text-xs text-slate-500">
               Avg. value: <span className="font-semibold">{formatPrice(metrics.avgOrderValue)}</span>
             </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm bg-white dark:bg-slate-900">
          <CardContent className="p-5">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Cash / UPI</p>
                <div className="space-y-1 mt-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Cash:</span>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{formatPrice(metrics.cashPayments)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Digital:</span>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{formatPrice(metrics.digitalPayments)}</span>
                  </div>
                </div>
              </div>
        ...
        <Card className="border-none shadow-sm bg-white dark:bg-slate-900">
          <CardContent className="p-5">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Tax & Discounts</p>
                 <div className="space-y-1 mt-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Tax:</span>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{formatPrice(metrics.totalTax)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Disc:</span>
                    <span className="font-semibold text-green-600 dark:text-green-400">-{formatPrice(metrics.totalDiscounts)}</span>
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {billingFilter !== 'all' && (
              <Badge variant="secondary" className="bg-blue-50 text-blue-700">
                Showing: {billingFilter === 'gst_invoice' ? 'GST Invoices' : 'Without GST Bills'}
              </Badge>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
            <Select value={billingFilter} onValueChange={setBillingFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="gst_invoice">GST Invoices</SelectItem>
                <SelectItem value="without_gst">Without GST Bills</SelectItem>
              </SelectContent>
            </Select>
                    ...
                    <TableRow key={sale.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors" onClick={() => handleViewDetails(sale)}>
                      ...
                      <TableCell>
                        {sale.billing_type === 'gst_invoice' ? (
                          <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-200">GST Invoice</Badge>
                        ) : (
                          <Badge variant="secondary" className="bg-slate-100 text-slate-700">Without GST</Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        <span className="font-mono text-sm text-slate-600 dark:text-slate-400">
                          {sale.billing_type === 'gst_invoice' 
                            ...
                            : (sale.bill_number || `BILL-${sale.id.substring(0, 8).toUpperCase()}`)}
                        </span>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="font-medium">{sale.customer_name || sale.point_of_sale_customers?.name || 'Walk-in Customer'}</span>
                          {sale.billing_type === 'gst_invoice' && sale.customer_gstin && <span className="text-xs text-blue-600 font-mono">GSTIN: {sale.customer_gstin}</span>}
                      ...
                      <TableCell className="text-right font-bold">
                        {formatPrice(sale.tax_breakdown?.grand_total || sale.final_amount || sale.total_amount)}
                      </TableCell>
